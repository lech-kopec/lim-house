class ApplicationController < ActionController::Base
  ...
  def home_page_records
    ...
    @received = ProdReturn.where(status: ApplicationHelper::ReturnsStatus[:przyjete]).paginate(page:params[:page], per_page:10)
      ...
      @rake_off_back = ProdReturn.where(status: ApplicationHelper::ReturnsStatus[:zwrot_prowizji]).paginate(page:params[:page], per_page:10)
      ...
      @transport_complains = ProdReturn.where(status: ApplicationHelper::ReturnsStatus[:rekl_przewoz]).paginate(page:params[:page], per_page:10)
    end
